...
trait TimeStatisticalsHelper
{
    ...
    private function getDataKeyByPgSQL(string $unit, string $field_time = "updated_at")
    {
        ...
        switch ($unit) {
            ...
            case "week":
                $format_date =
                    "concat(LPAD(extract (week from " .
                    $field_time .
                    ")::CHAR(2), 2, '0'),'-',extract (year from " .
                    $field_time .
                    "))";
                break;
            case "month":
    ...
    private function getTypeHanderCustom(
        $date_start,
        $date_end,
        string $unit = "day",
        string $field_time = "updated_at"
    ) {
        $typeHandle = ["unit" => $unit];
        $start = $date_start;
        $end = $date_end;
        $data = [
            "day_start" => $start->startOfDay(),
            "day_end" => $end->endOfDay(),
        ];
    ...
    private function _getTypeTimeStatisticalData(
        string $type,
        string $fieldTime = "updated_at",
    ...
    public function handleGetDataTimeStatistical(
        $typeHandle,
        Builder $query,
        array $data_default = [],
        string $field_time = "updated_at"
        ...
        $data_group = $query
            ->addSelect(DB::raw($formatDate . " as date"))
            ->orderBy("date")
            ->groupBy("date")
            ->get()
            ->groupBy("date");
    ...
    //
    public function handlerTimeStatistical(
        string $type,
        callable $cb,
        array $data_default = [],
        string $field_time = "updated_at"
