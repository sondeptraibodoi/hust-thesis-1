...
class BaseRepository extends EloquentRepository
{
    ...
    {
        return DB::transaction(function () use ($attributes, $cb, $action_log) {
            return $this->withLogHandler(
                "created",
                [
                    "description" => $this->defaultOption["logname"] . "." . $action_log,
                ],
                function ($model) use ($attributes, $cb) {
    ...
    {
        return DB::transaction(function () use ($id, $attributes, $cb, $action_log) {
            return $this->withLogHandler(
                "updated",
                [
                    "description" => $this->defaultOption["logname"] . "." . $action_log,
    ...
    {
        return DB::transaction(function () use ($id, $cb, $action_log) {
            return $this->withLogHandler(
                "deleted",
                [
                    "description" => $this->defaultOption["logname"] . "." . $action_log,
