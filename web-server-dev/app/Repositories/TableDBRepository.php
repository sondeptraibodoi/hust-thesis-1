    ...
    public function updateColumn($table_name, $column_info)
    {
        ...
        if ("string" === $column_info["type"]) {
            $queryBuilder->statement(
                "ALTER TABLE $table_name ALTER COLUMN \"{$column_info["name"]}\" TYPE CHARACTER VARYING USING \"{$column_info["name"]}\"::character varying"
            );
        } elseif ("text" === $column_info["type"]) {
            $queryBuilder->statement(
                "ALTER TABLE $table_name ALTER COLUMN \"{$column_info["name"]}\" TYPE TEXT USING \"{$column_info["name"]}\"::text"
            );
        } elseif ("integer" === $column_info["type"]) {
            $queryBuilder->statement(
                "ALTER TABLE $table_name ALTER COLUMN \"{$column_info["name"]}\" TYPE INTEGER USING \"{$column_info["name"]}\"::integer"
            );
        } elseif ("bigint" === $column_info["type"]) {
            $queryBuilder->statement(
                "ALTER TABLE $table_name ALTER COLUMN \"{$column_info["name"]}\" TYPE BIGINT USING \"{$column_info["name"]}\"::bigint"
            );
        } elseif ("double" === $column_info["type"]) {
            $queryBuilder->statement(
                "ALTER TABLE $table_name ALTER COLUMN \"{$column_info["name"]}\" TYPE DOUBLE PRECISION USING \"{$column_info["name"]}\"::double precision"
            );
        } elseif ("date" === $column_info["type"]) {
            $queryBuilder->statement(
                "ALTER TABLE $table_name ALTER COLUMN \"{$column_info["name"]}\" TYPE TIMESTAMP USING \"{$column_info["name"]}\"::timestamp"
            );
        }
    }
