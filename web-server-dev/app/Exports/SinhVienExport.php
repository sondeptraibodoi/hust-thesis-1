...
class SinhVienExport implements WithEvents, WithStyles
{
    use Exportable;
            ...
            AfterSheet::class => function (AfterSheet $event) {
                ...
                for ($i = 2; $i <= 6; $i++) {
                    $event->sheet
                        ->getDelegate()
                        ->getStyle("A$i:E$i")
                        ->applyFromArray($sub_title_style);
                }
                ...
                foreach ($subtitles as $key => $subs) {
                    foreach ($subs as $sKey => $sub) {
                        ...
                        if ($key == 0) {
                            ...
                            $event->sheet->mergeCells("B$row_idx:D$row_idx");
                        } else {
                            $event->sheet->setCellValue("E$row_idx", $sub);
                        }
                    }
                }
                ...
                $event->sheet->setCellValue("B5", Carbon::parse($this->sub_data["date"] ?? "")->format("d-m-Y"));
                ...
                foreach ($this->data as $key => $item) {
                    ...
                    // $event->sheet->setCellValue("C{$idx}", Carbon::parse($item['birthday'] ?? '')->format('d-m-Y'));
