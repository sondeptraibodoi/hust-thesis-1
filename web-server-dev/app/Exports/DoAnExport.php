...
class DoAnExport implements WithEvents, WithStyles
{
    use Exportable;
    private $file = "danh_sach_do_an.xlsx";
    private $data;
    private $sub_data;
    private $sheet_name;
    private $headers;
    ...
    {
        $this->file = "danh_sach_do_an.xlsx";
        $this->data = $data;
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->setCellValue("A1", "Danh sách đồ án");
                ...
                foreach ($this->data as $key => $item) {
                    ...
                    $event->sheet->setCellValue("G{$idx}", $item->ten_de_tai);
                    $event->sheet->setCellValue("H{$idx}", $item->noi_dung);
                    $event->sheet->setCellValue("I{$idx}", $item->cac_moc_quan_trong);
                    $event->sheet->setCellValue("J{$idx}", $item->giao_vien);
                }
            },
        ];
    }
