...
class Tracking
{
    ...
    public function handle($request, Closure $next, $type = "view-home")
    {
        ...
        $is_tracking = DB::table("tracking_views")
            ...
            ->where("date_time", ">=", Carbon::now()->subMinute(15))
            ->exists();

        $is_tracking = DB::table("ip_infos")->where("ip", $ip)->exists();
        if (!$is_tracking) {
            ...
            if ($position) {
                ...
                if (!empty($location["longitude"]) && !empty($location["latitude"])) {
                    $province = DB::selectOne(
                        "select id from provinces where ST_Contains( geometry::geometry,ST_SetSRID(ST_Point(" .
                            $location["longitude"] .
                            "," .
                            $location["latitude"] .
                            "), 4326))"
