...
class ImportSinhVienLopThiController extends Controller
{
    public function importSinhVienLopThi(Request $request)
    {
        ...
        try {
            ...
            $lop_this = LopThi::where("loai", "=", $loai)
                ->whereHas("lop", function ($query) use ($ki_hoc) {
            ...
            foreach ($items as $item) {
                ...
                if (!$lop_hoc) {
                    ...
                    return response()->json(
                        [
                            "message" => "Lớp $ma_lop_hoc không tồn tại trong dữ liệu",
                        ],
                        404
                    );
                }
                ...
                if (!$lop_thi) {
                    $lop_thi = LopThi::create([
                        "lop_id" => $lop_hoc["id"],
                        "ma" => $res["ma_lop_thi"],
                        "loai" => $loai,
                    ]);
                    $lop_thi = $lop_thi["id"];
                    $lop_this[$ma_lop_thi] = $lop_thi;
