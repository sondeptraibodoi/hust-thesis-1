        ...
        try {
            ...
            foreach ($items as $item) {
                ...
                $lop = Lop::updateOrCreate(
                    ["ma" => $res["ma"], "ki_hoc" => $ki_hoc],
                    array_merge($res, ["is_dai_cuong" => $is_dai_cuong, "loai_thi" => $loai_thi])
                );
                if (isset($key_giao_vien)) {
                    $giao_vien_emails = explode(",", $item[$key_giao_vien]);
                    $giao_vien_emails = array_reduce(
                        $giao_vien_emails,
                        function ($acc, $cur) {
                            $cur = trim($cur);
                            if (!empty($cur)) {
