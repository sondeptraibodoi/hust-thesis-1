    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            foreach ($items as $item) {
                ...
                $lophocs = Lop::where("ma_hp", $res["ma_hp"])
                    ->where("ma", $res["ma_lop"])
                    ->get();
                foreach ($lophocs as $lophoc) {
                    $lopthi = LopThi::where("lop_id", $lophoc->id)
                        ->where("ma", $malopthi)
                        ->first();
                    if (!$lopthi) {
                ...
                $is_exists = DiemPhucKhao::where("lop_thi_id", $lopthi->id)
                    ->where("sinh_vien_id", $id_sinh_vien->id)
                    ->exists();
                if ($is_exists) {
                    abort(400, "Điểm phúc khảo môn này đã tồn tại trong kỳ này");
                }
                DiemHelper::updateDiemPhucKhao($lopthi->id, $id_sinh_vien->id, $diem, [
