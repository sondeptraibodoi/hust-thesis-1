...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = QueryBuilder::for($query, $request)
            ...
            ->allowedSorts(["username", "inactive"])
            ->allowedFilters([
                "username",
                "inactive",
    ...
    {
        $query = User::query()->with(["hocPhanQuanLy"]);
        $query = QueryBuilder::for($query, $request)
            ->allowedSearch(["username"])
            ->allowedAgGrid([])
            ->defaultSort("created_at")
    ...
    public function store(UserRequest $request)
    {
        ...
        } elseif (in_array(RoleCode::STUDENT, $request->roles)) {
        ...
        if (array_search("hp_assistant", $request->roles) >= 0) {
            $user->hocPhan()->sync($request->get("hoc_phan"));
        }
    ...
    {
        $user = User::findOrFail($id);
        if (array_search("hp_assistant", $user->roles) >= 0) {
            HocPhanUser::query()->where("user_id", $id)->delete();
        }
        if (isset($user->info_id) && $user->is_giao_vien) {
