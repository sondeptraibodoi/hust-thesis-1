...
class QuenMatKhauController extends Controller
{
    public function postEmail(Request $request)
    {
        ...
        Mail::send([], [], function ($message) use ($username, $resetLink) {
            ...
            $message->setBody(
                view("emails.check_email_forget", [
                    "resetLink" => $resetLink,
                    "username" => $username,
                ])->render(),
                "text/html"
    ...
    public function resetPassword(Request $request)
    {
        ...
        if (!$data) {
            return response()->json(
                [
                    "message" => "Đường dẫn đã quá hạn. Vui lòng tạo đường dẫn mới để thiết lập mật khẩu mới",
                ],
                400
            );
        }
        if (Hash::check($data->email, $request["token"])) {
            $limitedTime = Carbon::parse($data->created_at)
                ->addHours(24)
                ->toDateTimeString();
            $currentTime = Carbon::now()->toDateTimeString();
