...
{
    use ResponseType;
    protected $includes = ["giaoViens", "sinhViens", "lanDiemDanhs", "lanDiemDanhs"];
    ...
    {
        $query = Lop::query()->with("maHocPhan", "lopSinhVienDoAn");
        ...
        $request->validate(
            [
                "ma" => [
                    ...
                    "max:255",
                    "min:1",
                    Rule::unique("ph_lops")->where(function ($query) use ($request) {
    ...
    public function addSinhVien(Request $request, $id)
    {
        ...
        try {
            ...
            foreach ($children as $item) {
                $other_class = LopSinhVien::query()
                    ->where("lop_id", $item["id"])
                    ->where("sinh_vien_id", $sinh_vien_ids);
                ...
                if ($other_class->get()->count()) {
                    return response()->json(
                        [
                            ...
                            "errors" => [
                                "sinh_viens" => ["sinh viên này đã tồn tại trong lớp $ma_other_class"],
                            ],
                        ],
                        422
                    );
            ...
            $sinhvien_exists = LopSinhVien::where("lop_id", $lop->id)
                ->where("sinh_vien_id", $sinh_vien_ids)
                ->exists();
            $next_sinh_vien_greater = LopSinhVien::where("lop_id", $lop->id)
                ->where("stt", $stt + 1)
                ->exists();
            $next_sinh_vien_equal = LopSinhVien::where("lop_id", $lop->id)
                ->where("stt", $stt)
                ->exists();

            if ($sinhvien_exists) {
                return response()->json(["error" => "Sinh viên đã tồn tại trong lớp"], 400);
            }
            ...
            $diem_chuyen_can = DiemChuyenCanHelper::tinhDiem($lop, $sinh_vien_ids, $lop->id);
            $lop->sinhViens()->syncWithoutDetaching([
    ...
    public function lopGiaoVien(Request $request)
    {
        ...
        $query = Lop::query()
            ->whereHas("giaoViens", function ($query) use ($user) {
    ...
    public function listLopDiemDanh(Request $request)
    {
        ...
        foreach ($query as $lop) {
            ...
            $dong_lan = Setting::where("setting_name", "dong_diem_danh_lan_" . $lan_diem_danh)
                ->where("ki_hoc", $ki_hoc)
                ->first();
            ...
            $lop["ngay_mo_setting"] = !empty($lich_hoc[$convert_dong_mo[0] - 1])
                ? SettingHelper::getT2TuanHocThuN($ngay_bat_dau, $lich_hoc[$convert_dong_mo[0] - 1])
                : null;
            $lop["ngay_dong_setting"] = !empty($lich_hoc[$convert_dong_mo[1] - 1])
                ? SettingHelper::getT2TuanHocThuN($ngay_bat_dau, (int) $lich_hoc[$convert_dong_mo[1] - 1] + 1)
                : null;
        }
        if (isset($filter_tuan)) {
            $result = $query->where("tuan_dong", $filter_tuan);
        } elseif (!isset($filter_tuan)) {
            $result = $query;
        }
        return response()->json(new \App\Http\Resources\Items($result), 200, []);
    }

    public function addSinhVienDoAn(Request $request, $id)
    {
        $request->validate(
            [
                "stt" => "required|integer|min:1",
        ...
        try {
            ...
            // foreach ($giao_viens_ids as $giao_vien_id) {
            LopSinhVienDoAn::create([
                "lop_id" => $id,
                "sinh_vien_id" => $sinh_vien_ids,
