    ...
    public function indexAgGird(Request $request)
    {
        ...
        $query = DB::query()->fromSub(function ($query) use ($user) {
            $query
                ...
                ->join("ph_lop_thi_giao_viens", "ph_lop_thi_giao_viens.lop_thi_id", "ph_lop_this.id")
            ...
            // });
            $query->where("ph_lop_thi_giao_viens.giao_vien_id", $user->info_id);

            $query->orderBy("ngay_thi", "desc");
            $query = $query->orderByRaw("CAST(substring(substring(kip_thi from '^(\\d+)'), '^(\\d+)') as INTEGER)");
        }, "lop_thi");
        $query->orderBy("ngay_thi", "desc");
        $query = $query->orderByRaw("CAST(substring(substring(kip_thi from '^(\\d+)'), '^(\\d+)') as INTEGER)");
