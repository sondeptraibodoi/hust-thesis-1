...
class LopThiSinhVienController extends Controller
{
    ...
    public function lopThiSinhVien($id)
    {
        $query = LopThiSinhVien::query()
            ...
            ->with(["lopThi", "sinhVien"]);
        return response()->json(new \App\Http\Resources\Items($query->get()), 200, []);
    }
    public function listLopThiSinhVien(Request $request, $id)
    {
        $query = LopThiSinhVien::query()->where("lop_thi_id", $id);
    ...
    public function deleteSinhVienLopThi(Request $request, $id)
    {
        ...
        try {
            DB::beginTransaction();
            $sinh_vien_delete = LopThiSinhVien::query()
            ...
            $next_sv_greater = LopThiSinhVien::query()
                ->where("lop_thi_id", $lop_thi_id)
                ->where("stt", ">", $sinh_vien_delete->stt)
                ->get()
                ->toArray();
            $is_exist_sv_equal = LopThiSinhVien::query()
                ->where("lop_thi_id", $lop_thi_id)
                ->where("stt", "=", $sinh_vien_delete->stt)
                ->exists();

            if (
                !empty($next_sv_greater) &&
                ($next_sv_greater[0]["stt"] > $sinh_vien_delete->stt && !$is_exist_sv_equal)
            ) {
                LopThiSinhVien::query()
                    ->where("lop_thi_id", $lop_thi_id)
                    ->where("stt", ">", $sinh_vien_delete->stt)
                    ->decrement("stt");
            }
