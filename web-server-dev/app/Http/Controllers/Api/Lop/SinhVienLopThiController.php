...
class SinhVienLopThiController extends Controller
{
    ...
    public function indexAgGird(Request $request)
    {
        ...
        $query = DB::query()->fromSub(function ($query) use ($user) {
            $query
                ...
                ->orderBy("ph_lop_this.kip_thi")
                ->orderBy("ph_lop_this.ngay_thi", "desc")
                ->select([
                    "ph_lop_this.id",
                    "ph_lop_this.ma",
                ...
                ->whereExists(function ($subquery) use ($user) {
                    $subquery
                        ->from("ph_lop_thi_sinh_viens")
                        ->where(function ($query) {
                            $query
                                ->whereColumn("ph_lop_thi_sinh_viens.lop_thi_id", "ph_lop_this.id")
                                ->orWhereColumn("ph_lop_thi_sinh_viens.lop_thi_goc_id", "ph_lop_this.id");
                        })
                        ->where("ph_lop_thi_sinh_viens.sinh_vien_id", $user->info_id);
                });
        }, "lop_thi");

        $query->orderBy("ngay_thi", "desc");
        $query->orderByRaw("CAST(substring(substring(kip_thi from '^(\\d+)'), '^(\\d+)') as INTEGER)");
