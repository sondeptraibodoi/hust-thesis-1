...
trait AddSortToQuery
{
    ...
    public function defaultSorts($sorts): self
    {
        if (empty($this->default_sort)) {
            $this->addFunctionToCallWhenGet(function () {
                ...
                if ($this->is_use_aggird && count($this->request->aggrid()["sortModel"]) > 0) {
                    // We've got requested sorts. No need to parse defaults.
                    return $this;
                }
                $sorts = is_array($this->default_sort) ? $this->default_sort : [$this->default_sort];
                $this->checkSorts($sorts)->each(function (AllowedSort $sort) {
                    $sort->sort($this);
                });
            });
        }
    ...
    {
        collect($sorts)->each(function (string $property) {
    ...
    {
        return $this->allowedSorts->first(function (AllowedSort $sort) use ($property) {
