...
class Array2XML
{
    ...
    /**
     * tag node use @tag or key of array
     * @children for subnodes
     * @attributes for attributes of node
     * @value for value ò node
     */
    /**
        ...
        if (is_array($arr)) {
            ...
            if (array_key_exists("@children", $arr)) {
                foreach ($arr["@children"] as $value) {
                    $tag = $value["@tag"];
                    $node->appendChild(self::convert($tag, $value));
                }
                unset($arr["@children"]);
            }
            ...
            elseif (array_key_exists("@value", $arr)) {
