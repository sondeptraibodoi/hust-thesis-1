...
class SendMaiSinhVienLopThiSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $sinh_viens->chunk(10, function ($items) use (&$current, $count) {
            try {
                $info_email =
                    "SEND MAIL NOTIFY - MSSV: " .
                    $items->pluck("mssv")->join(",") .
                    "  - DATE: " .
                ...
                Mail::to($items)->send(
                    new MailNotify([
                        ...
                        "content" =>
                            "Các bạn sinh viên thi ngày 5/11 nếu không có giấy tờ thì cần có mặt tại D3-106 trước kíp thi 25 phút để làm các thủ tục xác nhận thi",
                    ])
                );
            } catch (\Throwable $th) {
