...
function handleFormData(formData = new FormData(), value: any, key: string, keyArrayObject = null) {
  if (checkArray(value)) {
    ...
    Object.keys(value).forEach((e_key) => {
      formData = hasKeyArrayObject(formData, value[e_key], `${key}[${e_key}]`, keyArrayObject);
    });
...
function hasKeyArrayObject(formData: any, value: any, key: any, keyArrayObject: any) {
  if (keyArrayObject) {
    formData = handleFormData(formData, value[keyArrayObject], key, keyArrayObject);
  }
