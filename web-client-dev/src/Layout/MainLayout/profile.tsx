...
import { getAuthUser, getInfoAction, getInitData, logoutAction } from "@/stores/features/auth";
...
import { LaravelValidationResponse, Laravel400ErrorResponse } from "@/interface/axios/laravel";
...
export default function ProfileDrawer({ openState, closefunct }: ProfileDrawerProps) {
  ...
  const passwordFormOnFinish = async (value: any) => {
    ...
    try {
      ...
      if (is_handle) {
        api.error({
          message: "Thất bại",
          description: err.response.data?.errors?.password[0] || "Thay đổi mật khẩu thất bại"
        });
      }
    } finally {
      setLoading(false);
    ...
    return (
      <>
        {activeTab === "2" ? (
          ...
          <Button title="submit" loading={loading} type="default" htmlType="submit" block className="w-full">
            Thay đổi
          </Button>
        )}
      </>
        ...
        >
          <div>
            <Form.Item
              ...
              help={errorMessage?.errors?.["old_password"] ? errorMessage?.errors?.["old_password"][0] : undefined}
            >
              ...
              rules={[
                ...
                ({ getFieldValue }: any) => ({
                  validator(_: any, value: any) {
                    ...
                    return Promise.reject(new Error("Mật khẩu nhập vào không khớp"));
                  }
                })
              ]}
              validateStatus={validateForm("password")}
  ...
  return (
    <>
      ...
      >
        <Tabs activeKey={activeTab} onTabClick={handleTabClick} style={{ height: "100%" }} items={tabs}></Tabs>
