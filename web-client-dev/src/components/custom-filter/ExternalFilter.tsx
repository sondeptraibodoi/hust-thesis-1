...
const ExternalFilter = forwardRef(({ gridref }: ExternalFilterParams, ref) => {
  ...
  const avtiveFilterChanged = useCallback(
    (newValue: string) => {
      const filterInstance: any = gridref?.current.getFilterInstance("inactive");
    ...
    <Row style={{ padding: "0.5rem 0 2rem 0" }} gutter={[26, 0]}>
      <Col>
        <Space direction="vertical">
          ...
          <Search size="large" placeholder="Tìm theo tên" onSearch={externalFilterChanged} />
        </Space>
      </Col>
      <Col>
        <Space direction="vertical">
          <p style={{ fontSize: "1.2rem" }}>Lọc theo nhóm tài khoản</p>
          <Select size="large" placeholder="Chọn nhóm tài khoản" options={groupoption} onChange={groupFilterChanged} />
        </Space>
      </Col>
      <Col>
        <Space direction="vertical">
