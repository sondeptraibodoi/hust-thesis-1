...
const selectFilterGiaoVien = forwardRef((props: Props, ref) => {
  ...
  return (
    ...
    >
      ...
      >
        {renderOptionGiaoVien(props.data as any)}
      </Select>
    </div>
  );
});
export const renderOptionGiaoVien = (data: GiaoVien[]) => {
  ...
  return (
    <>
      {data.map((item, index) => (
        <Option key={index} value={item.id} label={`${item.name} (${item.email})`}>
          <p>{item.name}</p>
          <p>{item.email}</p>
        </Option>
      ))}
...
export const renderOptionGiaoVienId = (data: GiaoVien[]) => {
  if (!Array.isArray(data)) return <></>;
  if (!data || !data.length) return <></>;
  ...
  return (
    <>
      {data.map((item, index) => (
        <Option key={index} value={item.user?.id || item.id} label={`${item.name} (${item.email})`}>
          <p>{item.name}</p>
          <p>{item.email}</p>
        </Option>
      ))}
