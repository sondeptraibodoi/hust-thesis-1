...
const ImportExcelCompoment: FC<Props> = (props) => {
      ...
      if (appcectType != undefined) {
        ...
        if (isAcceptfile) {
          ...
          api.error({
            ...
            description:
              t("message.error_desc_unmatch_type_before") + file.name + t("message.error_desc_unmatch_type_after"),
            duration: 2
          });
        }
      } else {
  ...
  useEffect(() => {
    ...
    if (suggestList) {
      fieldName?.forEach((x) => {
        if (suggestList[x.name] != undefined && suggestList[x.name] instanceof Array) {
    ...
    <>
      ...
      >
        {buttonElement ? (
          ...
          <div
            style={{ ...buttonStyle, cursor: "pointer", width: 160 }}
            className={
              buttonClass +
              " bg-white w-full flex items-center justify-center max-h-[250px] h-full aspect-[3/4] border-gray-200 border-solid "
            }
          >
      ...
      {contextHolder}
      <Drawer open={opendrawer} onClose={closeDrawer} footer={<></>} className="relative">
        <Title level={5} className="mt-2">
          {t("upload.title")}
        </Title>
        <Form className="base-form" form={form} layout="vertical" onFinish={sendForm}>
          {extraFormItemCompoment?.position === "top" && extraFormItemCompoment.element}
                ...
                rules={
                  x.isRequired === false
                    ...
                    : [
                        {
                          required: true,
                          message: `Hãy nhập thông tin cho trường ${t(`field.${x.name}`).toLowerCase()}`
                        }
                      ]
                }
                label={x.text || t(`field.${x.name}`)}
                extra={x.ghi_chu}
              >
                <Select
                  allowClear
          ...
          {extraFormItemCompoment?.position === "below" && extraFormItemCompoment.element}
      ...
      {downloadable && (
        <Modal
          ...
          onOk={DownExcel}
        >
          <Table style={{ margin: "13px 15px 0" }} columns={columns} dataSource={returnValue} />
        </Modal>
      )}
    </>
  );
