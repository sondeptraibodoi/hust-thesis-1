...
const ModalExportThiBu: FC<Props> = ({ showModal, setShowModal, api, text }) => {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const res = await api({
        ...values,
        start_date: values.start_date ? dayjs(values.start_date).format("YYYY-MM-DD").toString() : null,
        end_date: values.end_date ? dayjs(values.end_date).format("YYYY-MM-DD").toString() : undefined
      });
    ...
    <>
      <Modal centered footer={<></>} className="relative" open={showModal} onCancel={() => handleCancel()}>
        ...
        <div className="modal-title-wapper">
          <p className="modal-title">Xuất danh sách thi bù</p>
        </div>
        <Form initialValues={{ role_code: "assistant" }} layout="vertical" className="" onFinish={onFinish} form={form}>
          <Form.Item label="Kì học" name="ki_hoc">
            <Select options={kiHoc}></Select>
          </Form.Item>
          <div className="flex justify-between gap-4">
            <Form.Item className="w-full" label="Từ ngày" name="start_date">
              <DatePicker className="w-full"></DatePicker>
            </Form.Item>
            <Form.Item className="w-full" label="Đến ngày" name="end_date">
              <DatePicker className="w-full"></DatePicker>
            </Form.Item>
          </div>
...
export default ModalExportThiBu;
