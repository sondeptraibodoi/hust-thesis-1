...
import { Laravel401Response, LaravelServerErrorResponse, LaravelValidationResponse } from "@/interface/axios/laravel";
import axios, { AxiosError, AxiosResponse } from "axios";

export function convertErrorAxios<T = any>(error: Error | AxiosError<T>): IAxiosError<T> | IJsError<T> {
...
export function axiosErrorHandler<T>(callback: (err: IAxiosError<T> | IJsError<T>) => void) {
...
export function isTokenInvalid(e: AxiosError<Laravel401Response>): boolean {
  return !!e.response && (e.response.status === 401 || e.response.data.message === "Unauthenticated.");
}
...
export function isServerError(e: AxiosError<LaravelServerErrorResponse>): boolean {
  return !!e.response && e.response.status === 500;
}

export function isServerInvalid(e: AxiosError<Laravel401Response>): boolean {
  return !!e.response && e.response.status > 500;
}
