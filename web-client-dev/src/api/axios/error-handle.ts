...
type ErrorHandlerFunction = (error?: THttpError) => ErrorHandlerObject | boolean | undefined | void;
...
class ErrorHandlerRegistry {
  ...
  handleError(seek: (string | undefined)[] | string, error: THttpError): boolean {
  ...
  responseErrorHandler(error: THttpError) {
    ...
    if (isAxiosError(error)) {
      ...
      const seekers = [
        ...
        // data?.code, //Our api can send an error code to you personalize the error message.
      ...
      return this.handleError(seekers, error);
...
export function dealsWith(solutions: ErrorHandlerMany, ignoreGlobal = false) {
