import { AxiosTransform, CreateAxiosOptions, RequestOptions } from "@/interface/axios/config";
...
const transform: AxiosTransform = {
  beforeRequestHook: (config: any, options: RequestOptions) => {
    const { urlPrefix, joinPrefix, withoutToken, authenticationScheme } = options;
    const { url } = config;
