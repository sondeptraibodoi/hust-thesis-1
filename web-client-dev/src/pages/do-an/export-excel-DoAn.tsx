...
const ModalExportDoAn: FC<Props> = ({ showModal, setShowModal, api, text }) => {
    ...
    <>
      <Modal centered footer={<></>} className="relative" open={showModal} onCancel={() => handleCancel()} width={570}>
        ...
        <div className="modal-title-wapper">
          <p className="modal-title">Xuất danh sách đồ án</p>
        </div>
        <Form
        ...
        >
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Vui lòng nhập kì học"
              }
            ]}
          >
            <Select allowClear placeholder={"Kì học"} options={kiHoc}></Select>
          </Form.Item>
...
export default ModalExportDoAn;
