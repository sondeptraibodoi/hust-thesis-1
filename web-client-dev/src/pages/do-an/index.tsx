...
import ModalExportDoAn from "./export-excel-DoAn";
const defaultColDef = {
  flex: 1,
  minWidth: 150,
  resizable: true,
...
const DoAnPageDesktop: FC<any> = () => {
  ...
  const [modalExportDoAn, setModalExportDoAn] = useState(false);
  ...
  const optionsEdit = [
    ...
    },
    {
      type: "input",
      name: "giao_vien",
      placeholder: t("field.giao_vien_hd"),
      label: t("field.giao_vien_hd"),
      readOnly: true
    },
    {
      type: "input",
  ...
  return (
    <>
      <PageContainer
        ...
        extraTitle={
          <Button onClick={() => setModalExportDoAn(true)} type="primary" style={{ float: "right" }}>
            Xuất danh sách
          </Button>
        }
      >
      ...
      <CreateNEditDialog
        apiCreate={() => {}}
        apiEdit={thuctapApi.adminEditDoAn}
        options={optionsEdit}
        translation={"do-an"}
        data={data}
        isEdit={isEdit}
      ...
      <ModalExportDoAn
        translation="do-an"
        showModal={modalExportDoAn}
        setShowModal={setModalExportDoAn}
        api={thuctapApi.excelDoAn}
        data={data}
        text="danh-sach-do-an"
      />
    </>
  );
};
const DoAnPageMobile: FC<{ setKeyRender: any }> = ({ setKeyRender }) => {
  const { t } = useTranslation("do-an");
  const [isEdit, setIsEdit] = useState<boolean>(false);
  ...
  const getData = useCallback(async (filter: any) => {
    setLoading(true);
    try {
      const res = await thuctapApi.listDoAnPage(filter);
          ...
          return (
            <Col span={24} key={record.id} className="my-2">
              <Card>
                ...
                <p className="my-1">
                  <strong>Giáo viên hướng dẫn:</strong> {record.giao_vien}
                </p>
                ...
                <div className="flex justify-center">
                  ...
                  <Tooltip title="Đánh giá">
                    <Link to={`/sohoa/lop-hoc/${record.lop_id}/${record.sinh_vien_id}/danh-gia`}>
...
const ActionCellRender: FC<any> = ({ onEditItem, data }) => {
  ...
  return (
    <>
      ...
      <Tooltip title="Đánh giá">
        <Link to={`/sohoa/lop-hoc/${data.lop_id}/${data.sinh_vien_id}/danh-gia`}>
