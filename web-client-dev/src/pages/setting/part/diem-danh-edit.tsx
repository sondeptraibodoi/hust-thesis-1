...
const DongDiemDanhDialog = ({ setKeyRender, openModal, closeModal, callnofi, data, api }: Props) => {
    ...
    api &&
      (await api({ ...value, id: data?.["id"] })
        ...
        .catch((err) => {
          callnofi.error({
            message: "Thất bại",
            description: err?.response.data.message || "Cài đặt ngày đóng điểm danh thất bại"
          });
        })
        .finally(() => {
          setLoading(false);
