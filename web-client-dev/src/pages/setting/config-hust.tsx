  ...
  useEffect(() => {
    if (setting && setting.config) {
      form.setFieldsValue({
        ...
        day_start_week_1: dayjs(setting.config?.day_start_week_1, "YYYY-MM-DD").utc().local(),
        day_start_week_1_do_an:
          setting.config?.day_start_week_1_do_an &&
          dayjs(setting.config?.day_start_week_1_do_an, "YYYY-MM-DD").utc().local(),
  ...
  const onFinish = async (value: any) => {
    ...
    try {
      if (value.day_start_week_1) {
        value.day_start_week_1 = dayjs(value.day_start_week_1).format("YYYY-MM-DD");
      }
  ...
  return (
    <>
      ...
      <Card style={{ maxWidth: "550px" }}>
        <Form form={form} {...layout} onFinish={(value) => onFinish(value)} labelWrap>
          <Form.Item name="ki_hoc" label="Kì học hiện tại">
            <Select style={{ maxWidth: "160px" }} options={kiHoc} />
          </Form.Item>
          <Form.Item name="day_start_week_1" label="T2 tuần 1 của năm học">
