...
import { Button, Table, Typography, Space, Card, Tag } from "antd";
          ...
          <Table pagination={false} dataSource={dataSource} loading={loading} rowKey="id" style={customTableStyle}>
            ...
            <Column
              ...
              render={(_: any, record: any) => {
                const res = !record.ngay_thi ? "" : format(new Date(record.ngay_thi), "dd/MM/yyyy");
                return <Space size="middle">{res}</Space>;
              }}
            />
            <Column
              title="Trạng thái soát điểm"
              dataIndex="soat_diem"
              key="soat_diem"
              align="center"
              render={(_: any, record: any) => {
                const res =
                  record.diems_count > 0 ? (
                    <Tag color="success" className="font-medium text-sm w-32 text-center">
                      Đã soát điểm
                    </Tag>
                  ) : record.id_nhan_dien ? (
                    <Tag color="warning" className="font-medium text-sm w-32 text-center">
                      Chưa soát điểm
                    </Tag>
                  ) : (
                    <Tag className="font-medium text-sm w-32 text-center">Chưa có điểm</Tag>
                  );
                return <Space size="middle">{res ? res : ""}</Space>;
              }}
            />
            <Column
              title="Hành động"
