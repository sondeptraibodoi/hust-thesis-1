...
const LopHocListDiemDanhPage: FC<{ lop: Lop }> = ({ lop }) => {
  ...
  const onCreateItem = () => {
    setData({ lop_id: lop.id, ngay_diem_danh: dayjs().tz("Asia/Ho_Chi_Minh") } as any);
    setIsCreate(true);
    setIsEdit(false);
  };
      ...
      {isMobile ? (
        <>
          ...
          <Table pagination={false} dataSource={dataSource} loading={loading} rowKey="id" style={customTableStyle}>
            <Column title="Lần" dataIndex="lan" key="lan" align="center" width={80} />
            <Column
      ...
      <TaoDiemDanh
        api={(data: any) =>
          lanDiemDanhApi.create({
            ...
            lop_id: lop.id,
            is_diem_danh_them: data.is_diem_danh_them
          })
        }
        translation={"lan-diem-danh"}
        data={data}
