...
import { Checkbox, Col, Row, Statistic, Table, notification, Card, Button, Modal } from "antd";
...
const DiemDanhListSinhVien: FC<{ lan_diem_danh: LanDiemDanh; lop: Lop }> = ({ lan_diem_danh, lop }) => {
  ...
  const onChangeCoMat = (e: CheckboxChangeEvent, record: DiemDanhItem) => {
    ...
    const index = updatedDataSource.findIndex((item) => item.diem_danh_id === record.diem_danh_id);
    updatedDataSource[index] = updatedRecord;
    setDataSource(updatedDataSource);
  };

  const onChangeGhiChu = (e: React.ChangeEvent<HTMLTextAreaElement>, record: DiemDanhItem) => {
    const updatedDataSource = [...dataSource];
  ...
  return (
    <>
      ...
      {authUser?.role_code === ROLE_CODE.TEACHER && (
        <Row align="middle" className="ngay-diem-danh">
          <Col xs={24} sm={24} md={12} lg={12}>
            <Statistic loading={loading} title={"Tổng số học sinh: "} value={dataSource.length} />
          </Col>
          <Col xs={24} sm={24} md={12} lg={12}>
            <Statistic
              loading={loading}
      ...
      {isMobile ? (
        <Table dataSource={dataSource} rowKey="diem_danh_id" pagination={false} loading={loading} scroll={{ y: 500 }}>
          <Column title="STT" dataIndex="stt" key="stt" width="8%" align="center" />
          <Column title="MSSV" dataIndex="mssv" key="mssv" width="10%" align="center" />
          <Column title="Tên sinh viên" dataIndex="name" key="name" width="10%" align="center" />
          <Column title="Lớp" dataIndex="group" key="group" width="10%" align="center" />
          <Column title="Mã Lớp" dataIndex="lop_ma" key="lop_ma" width="10%" align="center" />
          <Column
            title="Có mặt"
            dataIndex="co_mat"
            key="co_mat"
