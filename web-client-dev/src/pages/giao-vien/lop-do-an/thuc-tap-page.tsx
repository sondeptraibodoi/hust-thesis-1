...
const ThucTapPageDesktop: FC<any> = () => {
  ...
  return (
    <>
      <PageContainer title="Danh sách phiếu thực tập">
        <BaseTable
          columns={columnDefs}
          api={thuctapApi.listThucTapByGv}
...
const ThucTapPageMobile: FC<{ setKeyRender: any }> = ({ setKeyRender }) => {
  ...
  const getData = useCallback(async (filter: any) => {
    setLoading(true);
    try {
      const res = await thuctapApi.listThucTapByGv(filter);
  ...
  const onSubmit = (filter?: any) => {
    ...
    if (filter.ten_cong_ty) {
      sendData.filterModel.ten_cong_ty = {
        filterType: "text",
        type: "contains",
        filter: filter.ten_cong_ty
      };
    }
    if (filter.dia_chi) {
      sendData.filterModel.dia_chi = {
        filterType: "text",
        type: "contains",
        filter: filter.dia_chi
      };
    }
    if (filter.ghi_chu) {
