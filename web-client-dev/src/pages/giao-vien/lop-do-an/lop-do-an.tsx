...
const LopThucTapDoAnPage: FC<any> = () => {
  const [keyRender, setKeyRender] = useState(0);

  const contentDesktop = () => <LopThucTapDoAnPageDesktop />;
  const contentMobile = () => <LopThucTapDoAnPageMobile key={keyRender} setKeyRender={setKeyRender} />;
  return (
    <>
...
export default LopThucTapDoAnPage;

const LopThucTapDoAnPageDesktop: FC<any> = () => {
  ...
  return (
    <PageContainer title="Danh sách đồ án">
      <BaseTable
        columns={columnDefs}
        api={thuctapApi.listThucTapDoAn}
        gridOption={{ defaultColDef: defaultColDef }}
        key={keyRender}
      ></BaseTable>
      <CreateNEditDialog
        apiCreate={() => {}}
        apiEdit={thuctapApi.editDoAn}
        options={optionsEdit}
        translation={"do-an"}
        data={data}
        isEdit={isEdit}
...
const LopThucTapDoAnPageMobile: FC<{ setKeyRender: any }> = ({ setKeyRender }) => {
  const { t } = useTranslation("do-an");
  const [isEdit, setIsEdit] = useState<boolean>(false);
  ...
  const getData = useCallback(async (filter: any) => {
    setLoading(true);
    try {
      const res = await thuctapApi.listThucTapDoAn(filter);
