...
const LopHocListDiemDanhPage: FC<{ lop: Lop }> = ({ lop }) => {
  ...
  const contentDesktop = () => (
    <Table pagination={false} dataSource={dataSource} loading={loading} rowKey="id">
      ...
      <Column
        ...
        render={(_: any, record: LanDiemDanh | any) => {
          const res = !record.ngay_diem_danh ? "" : format(new Date(record.ngay_diem_danh), "dd/MM/yyyy");
      ...
      <Column
        ...
        render={(_: any, record: LanDiemDanh | any) => {
          const res = !record.ngay_mo_diem_danh ? "" : format(new Date(record.ngay_mo_diem_danh), "dd/MM/yyyy");
          return <Space size="middle">{res}</Space>;
        }}
      />
      <Column
        ...
        render={(_: any, record: LanDiemDanh | any) => {
          const res = !record.ngay_dong_diem_danh ? "" : format(new Date(record.ngay_dong_diem_danh), "dd/MM/yyyy");
          return <Space size="middle">{res}</Space>;
        }}
      />
      <Column
        ...
        render={(_: any, record: LanDiemDanh) => (
          <Space size="middle">
            <Link to={"diem-danh/" + record.id}>
              <Button shape="circle" icon={<OrderedListOutlined />} type="text" />
            </Link>
      ...
      {dataSource.map((record: LanDiemDanh) => (
        ...
        >
          <p>
            ...
            {record.ngay_diem_danh ? format(new Date(record.ngay_diem_danh.toString()), "dd/MM/yyyy") : ""}
          </p>
          <p>
            <strong>Ngày mở điểm danh: </strong>
            {record.ngay_mo_diem_danh ? format(new Date(record.ngay_mo_diem_danh.toString()), "dd/MM/yyyy") : ""}
          </p>
          <p>
            <strong>Ngày đóng điểm danh: </strong>
            {record.ngay_dong_diem_danh ? format(new Date(record.ngay_dong_diem_danh.toString()), "dd/MM/yyyy") : ""}
          </p>
        </Card>
      ))}
    </div>
  ...
  return (
    <>
      ...
      <DeleteDialog
        ...
        name="lần điểm danh"
        apiDelete={() => data && lanDiemDanhApi.delete(data, { lop_id: lop.id })}
        setKeyRender={setKeyRender}
      />
      <TaoDiemDanh
        openModal={isCreate}
