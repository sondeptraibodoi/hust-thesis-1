...
const { Option } = Select;
const EditorStudentDoAnDialog: FC<Props> = ({
  showModal,
  setShowModal,
  setKeyRender,
  isEdit,
  ...
  const getOtherCLass = (data: any) => {
    ...
    setOtherClass(() => data.children.filter((itemA: Lop) => lop.id !== itemA.id));
  };
  ...
  const getSinhVien = async (existStudent: SinhVien[]) => {
    try {
      ...
      const datafilter = res.data.list.filter(
        (itemA: SinhVien) => !existStudent.some((itemB: SinhVien) => itemA.id === itemB.id)
      );
    ...
    if (isEdit) {
      try {
        await lopHocApi.updateSVDoAn({
          ...
          cac_moc_quan_trong: values.cac_moc_quan_trong,
          giao_viens: values.giao_viens
        });
        api.success({
      ...
      try {
        ...
        if (res.type === "axios-error") {
          ...
          api.error({
            ...
            description: response?.data.errors.stt ? response?.data.errors.stt : t("message.error_desc_create")
          });
        }
      } finally {
        setLoading(false);
  ...
  useEffect(() => {
    if (isEdit && data) {
      form.setFieldsValue({
        ...
        cac_moc_quan_trong: data.cac_moc_quan_trong,
        giao_viens: data.giaoVienHD?.id
      });
    }
  }, [form, data, isEdit]);
  useEffect(() => {
  ...
  useEffect(() => {
    const getGiaoVien = async () => {
      try {
        ...
        if (res.data && res.data.length > 0) setGiaoVienList(res.data);
      } catch (error) {
        api.error({
      ...
      <Modal centered footer={<></>} className="relative" open={showModal} onCancel={() => handleCancel()}>
        ...
        >
          ...
          <Form.Item
            ...
            validateStatus={validateForm(errorMessage?.errors?.sinh_viens?.length)}
            help={errorMessage?.errors?.sinh_viens ? errorMessage?.errors?.sinh_viens[0] : undefined}
          >
            <Select disabled={isEdit} showSearch onSearch={handleChange} filterOption={false}>
              {renderOption(sinhVienList)}
            </Select>
          </Form.Item>

          <Form.Item label="Tên đề tài" name="ten_de_tai">
            <Input className="w-full bg-white"></Input>
          </Form.Item>

          <Form.Item label="Nội dung" name="noi_dung">
            <Input.TextArea className="w-full bg-white"></Input.TextArea>
          </Form.Item>

          <Form.Item label="Các mốc kiểm soát chính" name="cac_moc_quan_trong">
            <Input.TextArea className="w-full bg-white"></Input.TextArea>
          </Form.Item>
...
export default EditorStudentDoAnDialog;
...
const renderOptionGV = (data: GiaoVien[]) => {
