  ...
  return (
    <Fullcalendar
      ...
      events={event.map((event) => {
        const dayThi = dayjs(event.ngay_thi).format("YYYY-MM-DD") + "T";
        const timeParts = (event.kip_thi || "").split("-").map(formatTime);
        const paddedTimeParts = timeParts.map((timePart: { split: (arg0: string) => [any, any, any] }) => {
        ...
        return {
          title: `Mã lớp: ${event.ma}, Đợt thi: ${event.loai === "GK" ? "Giữa kỳ" : "Cuối kỳ"}`,
