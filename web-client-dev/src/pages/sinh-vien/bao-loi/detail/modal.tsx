...
const loai = [
  ...
  // { key: "option2", value: "Trạng thái thanh toán không đổi" },
  { key: "option3", value: "Khác" }
];
const hint_loai: Record<string, string> = {
  "Trạng thái thanh toán không đổi":
    "Vui lòng báo lỗi về hệ thống cùng với thông tin chuyển khoản (bao gồm tên tài khoản, mã chuyển khoản) và cần thông báo trong thời gian phúc khảo, trường hợp thông báo sau sẽ không được xử lý"
};
const FormAddNotifyError: FC<Props> = ({ showModal, setShowModal, setKeyRender, setEdit }) => {
  ...
  const [kiThiSelected, setKiThiSelected] = useState<string>();
  const { t } = useTranslation("bao-loi");
  const [loading, setLoading] = useState(false);
  const [loaiBaoLoi, setLoaiBaoLoi] = useState("");
  ...
  const handleChangeLoaiLoi = (name: any) => {
    setLoaiBaoLoi(hint_loai[name]);
  };
  ...
  const filteredClasses = lopData?.filter((item: any) => item?.ki_hoc === kiHocDachon);
      ...
      const fetchDataLopThi = async () => {
        try {
          const response = await lopHocErrorApi.getListLopThi({
            ki_hoc: kiHocDachon,
            lop: lopHocDachon,
            ki_thi: kiThiSelected
          });

          setLopThiData(response.data.data);
        } catch (error) {
          console.error("API call failed:", error);
        }
      };
  ...
  }, [lopHocDachon, kiHocDachon, kiThiSelected]);
  ...
  const handleChangKiThi = (value: any) => {
    setKiThiSelected(value);
  };
  return (
    <>
      {contextHolder}
      <Modal centered footer={<></>} className="relative" open={showModal} onCancel={() => handleCancel()}>
        ...
        >
          ...
          <Form.Item name="ki_thi" label="Kì thi" rules={[{ required: true, message: "Vui lòng chọn kì thi" }]}>
            <Select
              onChange={handleChangKiThi}
              className="w-full"
              options={[
                { label: "Giữa kì", value: "GK" },
                { label: "Giữa kì 2", value: "GK2" },
                { label: "Cuối kì", value: "CK" }
              ]}
            />
          </Form.Item>
          <Form.Item
            ...
            label="Lớp thi"
            rules={[{ required: true }]}
            validateStatus={validateForm(errorMessage?.errors?.lop_thi_id?.length)}
            help={errorMessage?.errors?.lop_thi_id ? errorMessage?.errors?.lop_thi_id[0] : undefined}
          >
            <Select
              allowClear
            ...
            >
              {lopThiData?.map((item) => (
                <Select.Option key={item.id} value={item?.id}>
                  {item?.ma_hp} - {item?.ten_hp} ({item?.ma_lop_thi})
                </Select.Option>
              ))}
            </Select>
          </Form.Item>
          ...
          {loaiBaoLoi && <Text strong>{loaiBaoLoi}</Text>}
          <Form.Item
            name={["baoLoi", "tieu_de"]}
            label="Lý do"
            rules={[{ required: true }]}
            validateStatus={validateForm(errorMessage?.errors?.tieu_de?.length)}
            help={errorMessage?.errors?.tieu_de ? errorMessage?.errors?.tieu_de[0] : undefined}
          >
            <Input />
          </Form.Item>
          ...
          <Form.Item>
            <div className="flex justify-between gap-4">
              <ColorButton block className="mr-4" onClick={() => handleCancel()}>
                Huỷ
              </ColorButton>
