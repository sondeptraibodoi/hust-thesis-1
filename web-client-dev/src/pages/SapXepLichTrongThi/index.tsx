import { App, Button, Empty, Form, Input, Modal, Select, Space, Table, Tooltip, Typography } from "antd";
import { CaretDownOutlined, CaretUpOutlined, SolutionOutlined } from "@ant-design/icons";
...
function getTimeInterval(startTime: string, endTime: string) {
  ...
  const interval = dayjs().hour(0).minute(minutes);
  return interval.format("HH:mm");
}

function countNearbyDuplicatesAndMark(array: any[], InputAndOutKeys: { comperaKey: string; outputKey: string }[]) {
...
function toHoursAndMinutes(time1: string, time2: string, method: "subtract" | "plus") {
  if (time1 && time2) {
    const hour = dayjs(time1, "HH:mm").hour();
    const minute = dayjs(time1, "HH:mm").minute();
    const hour2 = dayjs(time2, "HH:mm").hour();
    const minute2 = dayjs(time2, "HH:mm").minute();
...
export default function SapXepLichTrongThiPage() {
  ...
  const [loaiLopSelected, setLoaiLopSelected] = useState<number>(1);
  const [formTitle] = Form.useForm();
  const navigate = useNavigate();
  const { message, notification } = App.useApp();
  const [tongSoGiaoVien, setTongSoGiaoVien] = useState<number>(0);
  const [tongSoPhongThi, setTongSoPhongThi] = useState<number>(0);
  ...
  useEffect(() => {
    setLopthiHeader([
      ...
      { title: t("field.ma_hp"), dataIndex: "ma_hp" },
      {
        ...
        render: (_v, r) => (
          <p>
            {r.ma} <span>{`(${r.sl_sinh_vien != null ? r.sl_sinh_vien : 0})`}</span>
          </p>
        )
      },
      // {
      ...
      {
        title: (
          <Tooltip placement="topLeft" title={"(Số lượng giám thị/Số lượng lớp thi/Số lượng phòng thi)"}>
            {t("field.giam_thi")} ({tongSoGiaoVien}/{listLopThiTemp.length}/{tongSoPhongThi})
          </Tooltip>
        ),
          ...
          return (
            <ReactSortable
              ...
              onAdd={(x) => {
                ...
                if (
                  listLopThi.every((lt) => {
                    if (lt.kip_thi === r.kip_thi && lt.loai === r.loai && lt.phong_thi !== r.phong_thi) {
                  ...
                  if (
                    ...
                    setlistGiangvien((pre) =>
                      pre.map((GV) => {
                        if (GV.id === id) {
                          const hours = r.kip_thi.split("-").map((d) => dayjs(`${d}`, "hh[h]mm").format("hh:mm:ss"));
                          const intervalTime = getTimeInterval(hours[0], hours[1]);

                          return {
                            ...GV,
                            so_lop_trong: GV.so_lop_trong + 1,
                            thoi_gian_trong: toHoursAndMinutes(GV.thoi_gian_trong, intervalTime, "plus"),
                            ma: [...GV.ma, r.ma]
                          };
                        } else {
                          return { ...GV };
                          ...
                          if (
                            !listLopThi.some((lt) => {
                              if (
                                ...
                                if (listGiamthidrop[lt.ma].some((x) => x.id === d.id)) {
                                  return true;
                                }
                            })
                          ) {
                              ...
                              pre.map((GV) => {
                                if (GV.id === d.id) {
                                  const hours = r.kip_thi
                                    .split("-")
                                    .map((d) => dayjs(`${d}`, "hh[h]mm").format("hh:mm:ss"));

                                  const intervalTime = getTimeInterval(hours[0], hours[1]);
                                  return {
                                    ...GV,
                                    so_lop_trong: GV.so_lop_trong + -1,
                                    thoi_gian_trong: toHoursAndMinutes(GV.thoi_gian_trong, intervalTime, "subtract"),
  ...
  }, [reset, listLopThi, listGiamthidrop, tongSoGiaoVien]);
  ...
  const getLoptheokiHoc = async () => {
    ...
    const sortedList = listLoaiThi.data.sort((a, b) => {
      if (dayjs(a.ngay_thi).isAfter(b.ngay_thi)) {
        return 1;
      } else if (dayjs(a.ngay_thi).isBefore(b.ngay_thi)) {
        return -1;
      } else {
        ...
        if (
          new Date().setHours(parseInt(a.kip_thi.split(" ")[0].split("h")[0], 10)) >
    ...
    setKipThiValue(
      [...uniqueKipthi]
        .sort((a, b) => {
          ...
          try {
            return new Date().setHours(parseInt(a.split(" ")[0].split("h")[0], 10)) <
    ...
    setNgayThiValue([...uniqueNgayThi].sort().map((x) => ({ value: x, label: x })));

    setlistGiangvien((pre) => {
      const filstArr = pre.map((gv) => {
        return {
          ...gv,
          so_lop_trong: giaoVienMapping[gv.id] ? giaoVienMapping[gv.id].so_lop_trong || 0 : 0,
          thoi_gian_trong: "",
  ...
  const sortListGiamthi = (index: number, key: string, order: "asc" | "desc" | "") => {
    if (order === "desc") {
      if (index === 0) {
        setlistTempGiangvien((pre) => [
          ...pre.sort((a: { [index: string]: any }, b: { [index: string]: any }) => {
            if (
              a[key]
                ?.split(" ")
        ...
        setlistTempGiangvien((pre) => [
          ...pre.sort((a: { [index: string]: any }, b: { [index: string]: any }) => {
            if (
              a[key]?.split(" ")[a[key]?.split(" ")?.length - 1] > b[key]?.split(" ")[b[key]?.split(" ")?.length - 1]
            ) {
              return 1;
            } else if (
              a[key]?.split(" ")[a[key]?.split(" ")?.length - 1] < b[key]?.split(" ")[b[key]?.split(" ")?.length - 1]
  ...
  useEffect(() => {
    if (markHeader.every((x) => x === 0)) {
      ...
      sortListGiamthi(
        indexOFsort,
        indexOFsort === 3 ? "thoi_gian_trong" : indexOFsort === 2 ? "so_lop_trong" : "name",
  ...
  const filterListGiangthi = (value: string) => {
    setlistTempGiangvien(() => listGiangvien.filter((data) => data.name?.toLowerCase().includes(value.toLowerCase())));
  };
  ...
  const exportDS = async (value: any) => {
    try {
      const res = await exportApi.excelXepLichThi({
        ...
        is_dai_cuong: loaiLopSelected == 1,
        title: value.title ? String(value.title).toUpperCase() : undefined
      });
      ...
      link.setAttribute(
        "download",
        `Lich-coi-thi-giao-vien_${dateFilterValue || dayjs().format("DD-MM-YYYY")}_${kipThiFilterValue || ""}.xlsx`
      );
  ...
  const filterListLopthi = () => {
    setListLopThiTemp(() => {
      ...
      const filteredList = listLopThi.filter((x) => {
        if (dateFilterValue && dateFilterValue != "" && kipThiFilterValue && kipThiFilterValue != "") {
          return x.ngay_thi === dateFilterValue && x.kip_thi === kipThiFilterValue;
        } else {
      ...
      const groupedList = countNearbyDuplicatesAndMark(filteredList, [
        {
          comperaKey: "kip_thi",
          outputKey: "kip_thi_RowSpan"
        },
  ...
  useEffect(() => {
    const tongSoGiaoVien = listLopThiTemp.reduce((total, lop) => total + lop.giao_viens.length, 0);

    const phongThi = new Set(listLopThiTemp.map((lop) => `${lop.ngay_thi}-${lop.kip_thi}-${lop.phong_thi.trim()}`));
    const tongSoPhongThi = phongThi.size;
    setTongSoGiaoVien(tongSoGiaoVien);
    setTongSoPhongThi(tongSoPhongThi);
  }, [filterListLopthi]);
  ...
  const onSaveAndSend = async (value: any) => {
    ...
    try {
      const newArr = listGiangvien
        ...
        .filter((x) => x != null);
      await lopThiApi.giaoVienTrongThiSendMail({
        info: newArr,
        ki_hoc: kiHoc,
        loai: loaikyThi,
        title: value.title,
  ...
  const formatLoaiLop = function (value?: number) {
    switch (value) {
      case 1:
        return "Đại cương";
      case 2:
  ...
  return (
    <PageContainer title={t("title.main")}>
      <div className={isMobile ? "flex" : "flex flex-wrap"}>
        <div className="w-full">
          <div className="flex flex-wrap gap-2 2xl:justify-between me-2 mb-2">
            ...
            <Space className="bg-gray-200 border-solid border border-[#c0c0c0] py-2 px-4 rounded-lg flex flex-wrap">
              <Select
                className="me-1"
            ...
            <Space className="bg-gray-200 py-2 px-4 rounded-lg flex flex-wrap">
              <Select
                className="me-1"
                style={{ width: "9rem" }}
              ...
              <Select
                ...
                options={kipThiValue}
              />
              <Button disabled={listLopThiTemp.length > 1 ? false : true} onClick={filterListLopthi}>
                {t("action.filter")}
              </Button>
            </Space>
            <Space className="bg-gray-200 border-solid border border-[#c0c0c0] py-2 px-4 rounded-lg flex flex-wrap">
              ...
              <Button loading={loadingSaveAndSend} onClick={() => setDialogSendMail(true)} type="primary">
                {t("action.sendMail")}
              </Button>
              <Button loading={loadingSave} onClick={() => onSave(lopThiApi.SapXepLichTrongThiSave, 0)} type="primary">
                {t("action.save")}
              </Button>
            </Space>
          </div>
              ...
              <thead className="bg-[#eff0f3] rounded border-2 border-gray-700 nodrag">
                <tr>
                  ...
                  >
                    {t("field.ho")}{" "}
                    <span>
                      {markHeader[0] === 1 ? <CaretUpOutlined /> : markHeader[0] === -1 ? <CaretDownOutlined /> : ""}
                    </span>
                  </th>
                  <th
                    onClick={() => {
                  ...
                  >
                    {t("field.ten")}{" "}
                    <span>
                      {markHeader[1] === 1 ? <CaretUpOutlined /> : markHeader[1] === -1 ? <CaretDownOutlined /> : ""}
                    </span>
                  </th>
                  <th
                    onClick={() => {
                  ...
                  >
                    ...
                    <span>
                      {markHeader[2] === 1 ? <CaretUpOutlined /> : markHeader[2] === -1 ? <CaretDownOutlined /> : ""}
                    </span>
                  </th>
                </tr>
              </thead>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className=" border-solid whitespace-nowrap border border-[#c0c0c0] p-3 text-sm text-gray-700">
                            {gv?.name?.split(" ")[gv?.name?.split(" ")?.length - 1]}
                          </td>
      ...
      <Modal
        ...
        centered
      >
        <Form form={formTitle} onFinish={onSaveAndSend} className="pt-8" layout="vertical">
          <h2 className="text-center mb-4">Gửi mail cho giảng viên</h2>

          <Form.Item className="">
            <Text italic>
              Hệ thống sẽ gửi email thông báo cho các giảng viên về lịch trông thi các {formatLoaiLop(loaiLopSelected)}{" "}
              của học kì {kiHoc}
              {loaikyThi && <span> có loại {formatDotThi(loaikyThi)}</span>}
              {dateFilterValue && <span> vào ngày {dateFilterValue}</span>}
              {kipThiFilterValue && <span> lúc {kipThiFilterValue}</span>}
            </Text>
          </Form.Item>
          ...
          <Form.Item className="">
            <Text strong>Xin hãy chắc chắn rằng toàn bộ dữ liệu đã được lưu trước khi nhấn vào nút gửi.</Text>
          </Form.Item>
          <Form.Item className="">
            <div className="flex justify-between">
              ...
              <Button loading={loadingSaveAndSend} type="primary" htmlType="submit">
                Gửi
              </Button>
            </div>
          </Form.Item>
      ...
      <Modal
        ...
        centered
      >
        <Form form={formTitle} onFinish={exportDS} className="pt-8" layout="vertical">
