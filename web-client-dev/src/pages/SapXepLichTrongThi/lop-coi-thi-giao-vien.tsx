...
const LopCoiThiGiaoVien = () => {
  ...
  const filterTable = () => {
    ...
    if (ngayThiSelected && !kipThiSelected) {
      ...
      lopThiGV.forEach((item: LopThiGV) => {
        if (item.ngay_thi === ngayThiSelected && item.kip_thi === kipThiSelected) {
          newData.push(item);
        }
      });
    }
    ...
    setKipThi(
      listKipThi
        .sort((a: any, b: any) => {
          ...
          try {
            return new Date().setHours(parseInt(a.value.split(" ")[0].split("h")[0], 10)) <
              new Date().setHours(parseInt(b.value.split(" ")[0].split("h")[0], 10))
              ? -1
              : 1;
          } catch (error) {
            return 0;
