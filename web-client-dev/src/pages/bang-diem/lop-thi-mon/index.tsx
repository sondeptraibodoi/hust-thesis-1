...
import { Button, Tag, Tooltip, Form, Col, Select, Spin, Card, Pagination } from "antd";
...
const DanhSachLopThi = () => {
  ...
  const breadcrumbs = useMemo(() => {
    return [
      { router: "../../", text: "Danh sách bảng điểm" },
  ...
  useEffect(() => {
    ...
    setColumDefs([
      ...
      },
      {
        ...
        hide: authUser?.is_giao_vien ? true : false
      },
      {
  ...
  return (
    <>
      <BaseTable
        key={keyRender}
        columns={columnDefs}
        api={(params: CallbackParams) => lopThiApi.lopThiMon(params, bang_diem_id, authUser)}
      ...
      <DeleteDialog
        ...
        name={data?.tieu_de}
        apiDelete={() => data && bangDiemApi.deleteLopThi(bang_diem_id, data.lop_thi_id)}
...
const DanhSachLopThiMobile: FC<{
  ...
  const Filter = (
    <Form form={form} layout="vertical" {...layout} labelWrap onFinish={onSubmit}>
      <Form.Item className="col-span-12 sm:col-span-6 lg:col-span-3" name="lopThi" label="Mã lớp thi">
        <Select allowClear onChange={(value) => handleFieldChanged("ki_hoc", value)}>
          {lopThi.map((item) => (
            <Select.Option key={item.lop_thi?.id}>{item.lop_thi?.ma}</Select.Option>
          ))}
        </Select>
      </Form.Item>
    </Form>
  ...
  if (loading) {
    ...
    Content = <div className="p-2 text-center"> Giáo viên chưa có lớp thi nào</div>;
  } else {
    Content = (
      <>
            ...
            <Col span={24} key={record.id} className="my-2">
              <Card>
                ...
                <p className="my-1">
                  ...
                  {record.diem_count_not_null == record.diems_count && record.diems_count != 0 ? (
...
const GiaoVienCellRender: FC<any> = ({ data }) => {
  if (!data) return <></>;
  return (
    <>
      {data.lop_thi.lop.giao_viens.length > 0
        ? data.lop_thi.lop.giao_viens.map((item: any) => {
