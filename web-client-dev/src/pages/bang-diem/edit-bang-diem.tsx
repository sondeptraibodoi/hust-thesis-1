...
const BangDiemEditor: FC<Props> = ({ showModalEdit, setShowModalEdit, setKeyRender, data }) => {
  ...
  const onFinish = async (value: any) => {
    setLoading(true);
    if (value.file != null && value.file != undefined && value.file instanceof Object) {
  ...
  return (
    <>
      {contextholder}
      <Modal centered className="relative" open={showModalEdit} onCancel={onCancel} footer={null} width={570}>
        <div className="model-container">
          ...
          <div className="scrollable-content">
            <Form layout="vertical" className="base-form flex-grow-1 overflow-y-auto" onFinish={onFinish} form={form}>
              <Form.Item name="loai" label="Loại" rules={[{ required: true, message: "Loại không được để trống" }]}>
                <Select disabled={true} onChange={setSelectedValue}>
                  <Select.Option value="nhan_dien">Nhập tự động File PDF</Select.Option>
                  <Select.Option value="nhap_tay">Nhập tay File EXCEL</Select.Option>
                </Select>
              </Form.Item>
              <Form.Item
                label="Kì học"
              ...
              >
                <Select disabled={true} onChange={setKiHocSelected} options={kiHoc}></Select>
              </Form.Item>
              ...
              {selectedValue == "nhan_dien" ? (
                <>
                  ...
                  >
                    <Input disabled={true} onChange={() => handleChange("ma_hp")}></Input>
                  </Form.Item>
                  <Form.Item
                    label="Tên học phần"
                    name="ten_hp"
                ...
                <Form.Item name="lop_hoc" label="Lớp học">
                  <Select
                    showSearch
                    disabled={data?.isPhucKhao || dayjs().format("YY-D-M").toString() > data?.ngay_ket_thuc_phuc_khao}
