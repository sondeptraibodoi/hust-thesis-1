...
const ImportDiemPhucKhao: FC<Props> = (props) => {
  const {
    translation,
    fieldName,
  ...
  return (
    <>
      <Tooltip title={<span>Nhập file điểm phúc khảo</span>}>
        <Button
          onClick={() => {
      ...
      <Drawer open={opendrawer} onClose={closeDrawer} footer={<></>} className="relative">
        <Form className="base-form" form={form} style={{ maxHeight: "98%" }} layout="vertical" onFinish={sendForm}>
          <Form.Item name="bang_diem_id" initialValue={data.id} style={{ display: "none" }}>
            <Input disabled />
          </Form.Item>
          ...
          <Form.Item className=" px-6 pt-4 bg-white">
...
export default ImportDiemPhucKhao;
