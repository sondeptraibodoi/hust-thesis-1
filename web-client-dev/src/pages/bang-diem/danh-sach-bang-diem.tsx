...
const DanhSachBangDiem = () => {
  ...
  const contentDesktop = () => <DanhSachBangDiemDesktop kiHoc={kiHoc} key={keyRender} />;
  const contentMobile = () =>
    kiHienGio && <DanhSachBangDiemMobile kiHoc={kiHoc} kiHienGio={kiHienGio} key={keyRender} />;
  return (
    <>
      <PageContainer
        title="Quản lý bảng điểm môn học"
        extraTitle={
          <Button onClick={() => setShowModalBangDiem(true)} type="primary" style={{ float: "right" }}>
            Thêm bảng điểm
          </Button>
        }
      >
        <BaseResponsive contentDesktop={contentDesktop} contentMobile={contentMobile} />
        <BangDiemCreate showModal={showModalBangDiem} setShowModal={setShowModalBangDiem} setKeyRender={setKeyRender} />
      </PageContainer>
    </>
  );
};
...
const DanhSachBangDiemDesktop: FC<{ kiHoc: string[]; key: number }> = ({ kiHoc }) => {
  ...
  const [columnDefs, setColumDefs] = useState<ColDef<IBangDiem & ActionField>[]>([]);
...
const DanhSachBangDiemMobile: FC<{
  ...
  const Filter = (
    ...
    >
      <Row>
        ...
        <Col xs={24} sm={12} md={8} lg={8} xxl={6}>
          <Form.Item className="col-span-12 sm:col-span-6 lg:col-span-3" name="ki_thi" label="Đợt thi">
            <Select
              allowClear
              onChange={(value) => {
                pagination.page = 1;
                handleFieldChanged("ki_thi", value);
              }}
            >
              {dotThi.map((item) => (
  ...
  if (loading) {
    ...
    Content = <div className="p-2 text-center"> Giáo viên chưa có bảng điểm nào </div>;
  } else {
    Content = (
      <>
        {contextholder}
          ...
          return (
            <Col span={24} key={record.id} className="my-2">
              <Card>
                ...
                <p className="my-1">
                  <strong>Ngày công khai: </strong> {formatDate(record.ngay_cong_khai)}
                </p>
                <p className="my-1">
                  <strong>Ngày kết thúc phúc khảo: </strong> {formatDate(record.ngay_ket_thuc_phuc_khao)}
                </p>
                <p className="my-1">
                  <strong>Trạng thái nhận diện: </strong>
                  {record.trang_thai_nhan_dien == "success" ? (
                    <Tag color="#87d068">Hoàn thành</Tag>
                  ) : record.trang_thai_nhan_dien == "processing" ? (
                    <Tag color="#f50">Đang nhận diện</Tag>
                  ) : record.trang_thai_nhan_dien == "failed" ? (
                    <Tag color="error">Lỗi trong quá trình nhận diện</Tag>
                  ) : (
                    <Tag>Chưa nhận diện</Tag>
                  )}
                </p>
...
const ActionCellRender: FC<any> = ({ onTuDongNhap, onCongBo, onUpdateItem, onDeleteItem, data, tooltip }) => {
  if (!data) {
    return <span></span>;
  }
...
const CongBoModal = ({ openModal, translation, api, closeModal, setKeyRender, callnofi, data }: Props) => {
  ...
  const onFinish = async (value: any) => {
    ...
    api &&
      (await api({
        ...
        ngay_ket_thuc_phuc_khao: dayjs(value.ngay_ket_thuc_phuc_khao).format("YY-M-D").toString(),
        id: data?.["id"]
      })
        .then(() => {
          callnofi.success({
  ...
  useEffect(() => {
    form.setFieldsValue({
      ngay_cong_khai: data?.["ngay_cong_khai"] && dayjs(data?.["ngay_cong_khai"]),
      ngay_ket_thuc_phuc_khao: data?.["ngay_ket_thuc_phuc_khao"] && dayjs(data?.["ngay_ket_thuc_phuc_khao"])
    });
  }, [data]);
              ...
              rules={[
                ...
                ({ getFieldValue }) => ({
                  validator(_, value) {
                    ...
                    return Promise.reject(new Error("Ngày kết thúc không được phép bé hơn ngày công khai"));
                  }
                })
              ]}
            >
...
const TuDongNhapModal = ({ openModal, translation, closeModal, data, setKeyRender, callnofi, user }: Props) => {
  const [loai_thi, setLoaiThi] = useState("");
  const { t } = useTranslation(translation);
  const [loading, setloading] = useState(false);
  useEffect(() => {
    if (!data) {
    ...
    if (data.ki_thi.includes("GK")) {
      setLoaiThi("GK");
    } else {
      setLoaiThi(data.ki_thi);
    }
  }, [data]);
  const onFinish = async () => {
    ...
    try {
      ...
      callnofi.error({
        message: "Thất bại",
        description: err?.response.data.message || "Không thể thực hiện hành động"
      });
    } finally {
      setloading(false);
      setKeyRender(Math.random());
    ...
    >
      <div className="w-full">
        <Select
          ...
          className="w-full"
          options={[
            { value: "KH ", label: <span>Thang điểm 30</span> },
            { value: "GK", label: <span>Thang điểm 15</span> },
            { value: "CK", label: <span>Thang điểm 10</span> }
          ]}
        />
      </div>
...
const DateFormat: FC<any> = (data, key: string) => {
  ...
  let date: any;
  if (data[key]) date = dayjs(data[key]).format("DD/MM/YYYY");
