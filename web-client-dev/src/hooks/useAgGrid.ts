import { ApiListReturn, AxiosResponseListReturn, Paginate } from "@/interface/axios";
...
export class DataSource<T = any> implements IDatasource {
  ...
  setPagination(pagination: Partial<Paginate>) {
  ...
  getRows(params: IGetRowsParams) {
    ...
    if (this.filterModel && JSON.stringify(this.filterModel) != JSON.stringify(params.filterModel)) {
      page = 1;
    }
    ...
    this.callback({
      ...
      itemsPerPage,
      paginate: true,
      sortModel: this.params ? this.params.sortModel : params.sortModel,
      filterModel: this.params ? this.params.filterModel : params.filterModel,
      ...this.defaultParams
    })
      .then((res) => {
...
export type CallbackGetData<T = any> = (_params: CallbackParams) => Promise<AxiosResponseListReturn<T>>;
